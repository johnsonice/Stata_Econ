...
**Section Two** Syntax***
	...
	// operators  and:& or: | Not:!
		...
		*inrange function will do the same thing
		...
		// you can also use inlist function, if you have too many or conditions
		keep if inlist(code,111,112,146,158,170,532,576,924)  // only keep observations with code = 111,112..... 
...
*Section Three ** Data to Dataset ****
	...
	//transform date from string to stata date
		...
		gen year = year(available_new)
		gen month = month(available_new)
		
		// how to get quarter of a date variable
		*gen time = monthly(date,"YM")
		*format %tm time
		*gen qdate = qofd(dofm(time))
		*format %tq qdate
		*gen ydate = year(dofm(time))
...
************************
	...
	********************************************
	******Section Eleven: some examples using macros   *********	
	********************************************
		...
		// look through local variables using reference
