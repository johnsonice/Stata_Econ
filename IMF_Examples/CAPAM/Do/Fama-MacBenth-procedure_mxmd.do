...
local vars mxwd					// the x variable you want to use 
drop if missing(`vars')         // drop if x variable is missing
local y NGDPD_PCH				// the y variable you want to plug in  
...
* save betas as dta
clear
import excel "stg1_reg.xlsx", sheet("`y'") firstrow case(lower)
...
local y NGDPD_PCH                    // y variable you want to use
levelsof year, local(tt)
local vars beta						// mxwd,mxwo,sp500 have different length, here, not sure if we want to set the time frame the same as the length of the variable 
